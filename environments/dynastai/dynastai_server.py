...
# Extend BaseEnvConfig with our custom field
class DynastAIConfig(BaseEnvConfig):
    """DynastAI environment configuration"""

    data_file_path: Optional[str] = Field(
        default=None,
        description="Path to custom JSON file with card data. If not provided, defaults to dynastai_cards.json",
    )
...
class DynastAIEnv(BaseEnv):
    ...
    name = "dynastai"
    env_config_cls = DynastAIConfig  # Use our custom config class

    def __init__(
        self,
        config: DynastAIConfig,  # Updated type hint
    ...
    def config_init(
        cls,
    ) -> Tuple[DynastAIConfig, List[APIServerConfig]]:  # Updated return type
        env_config = DynastAIConfig(  # Use DynastAIConfig instead of BaseEnvConfig
            ...
            wandb_name="dynastai",
            data_file_path=None,  # Default None value for data_file_path
        )
        server_configs = [
            APIServerConfig(
    ...
    async def setup(self):
        ...
        if self.data_file_path and os.path.exists(self.data_file_path):
            # Use provided data file path if it exists
            cards_file = self.data_file_path
            print(f"[DYNASTAI] Using custom data file: {cards_file}")
        else:
            # Fall back to default path
            cards_file = os.path.join(os.path.dirname(__file__), "dynastai_cards.json")
            print(f"[DYNASTAI] Using default data file: {cards_file}")
    ...
    async def get_next_item(self) -> DynastAIRow:
        ...
        prompt = self.format_prompt(kingdom_state, choice_history)
        print(
            f"[DYNASTAI] Kingdom state - Piety: {kingdom_state.get('Piety', 50)}, Stability: {kingdom_state.get('Stability', 50)}, Power: {kingdom_state.get('Power', 50)}, Wealth: {kingdom_state.get('Wealth', 50)}"
        )
